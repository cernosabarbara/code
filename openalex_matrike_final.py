...
def fetch_papers(query, search_field="title.search"):
    papers = []
    for page in tqdm(range(1, 4), desc=f"'{query}' ({search_field})", leave=False):
...
# Assuming works_df has a column 'referenced_works' with lists of cited OpenAlex IDs
# Map OpenAlex IDs to your internal IDs (0 to 2536)
openalex_to_id = {row['openalex_id']: row['id'] for _, row in works_df.iterrows()}
...
# Analyze the distribution of co-authorship weights
weights = coauthor_matrix.data
print("Porazdelitev uteži (število skupnih člankov med avtorji):")
for w in range(1, 6):  # Check weights from 1 to 5
    count = np.sum(weights == w)
    print(f"Število parov z utežjo {w}: {count}")

# Create the co-authorship network with weight threshold >= 2
G_authors = nx.Graph()
for r, c, w in zip(coauthor_matrix.nonzero()[0], coauthor_matrix.nonzero()[1], coauthor_matrix.data):
    if r < c and w >= 2:  # Only add an edge if authors co-authored at least 2 papers
...
largest_component = max(components, key=len) if components else set()  # Handle case with no components
largest_subgraph = G_authors.subgraph(largest_component) if largest_component else nx.Graph()
...
# Create a DataFrame of authors and their clusters
...
legal_articles = df[df['mentions_legislation']][['title', 'year', 'abstract', 'keywords']]
legal_articles.to_csv('legal_articles.csv', index=False)
...
critical_articles = df[df['is_critical']][['title', 'year', 'abstract', 'keywords']]
critical_articles.to_csv('critical_articles.csv', index=False)
...
psych_articles = df[df['is_psych_study']][['title', 'year', 'abstract', 'keywords']]
psych_articles.to_csv('psych_articles.csv', index=False)


# Preverjanje omembe zakonodaje (already in your code)
legal_terms = ["gdpr", "GDPR", "AI Act"]
df['mentions_legislation'] = (
    df['abstract'].str.lower().str.contains('|'.join(legal_terms), na=False) |
    df['title'].str.lower().str.contains('|'.join(legal_terms), na=False)
...
# Preverjanje omembe demografskih sprememb
demog_terms = [
   "aging population", "aging", "elderly", "older adults", "ageing", "demographic aging", "demographics"
]
df['is_demog_study'] = (
    df['abstract'].str.lower().str.contains('|'.join(demog_terms), na=False) |
    df['title'].str.lower().str.contains('|'.join(demog_terms), na=False) |
    df['keywords'].apply(
        lambda ks: any(term in k.lower() for k in ks for term in demog_terms) if isinstance(ks, list) else False
    )
)
# Izpis rezultatov
print(f"\nŠtevilo demog študij: {df['is_psych_study'].sum()} člankov")
print("Vzorec demog študij:")
print(df[df['is_demog_study']][['title', 'year']].head(35))
demog_articles = df[df['is_demog_study']][['title', 'year', 'abstract', 'keywords']]
demog_articles.to_csv('demog_articles.csv', index=False)
...
# Filter for demographic studies
df['is_demog_study'] = (
    df['abstract'].str.lower().str.contains('|'.join(demog_terms), na=False) |
    df['title'].str.lower().str.contains('|'.join(demog_terms), na=False) |
    df['keywords'].apply(
...
# Print results
print(f"\nŠtevilo demog študij: {df['is_demog_study'].sum()} člankov")
print("Vzorec demog študij:")
print(df[df['is_demog_study']][['title', 'year', 'authors', 'journal']].head(35))

# Export to CSV
demog_articles = df[df['is_demog_study']][['title', 'year', 'authors', 'journal', 'abstract', 'keywords']]
demog_articles.to_csv('demog_articles3.csv', index=False)




# Seznam avtorjev iz referenc (brez tistih, povezanih z zakonodajo)
reference_authors = [
    "Ajunwa", "Crawford", "Schultz", "Atkinson", "Batagelj", "Mrvar", "Binns", "Briner", 
    "Brynjolfsson", "McAfee", "Carr", "Castro", "New", "Coeckelbergh", "Csikszentmihalyi", 
    "Deci", "Ryan", "Deleuze", "Floridi", "Cowls", "Beltrametti", "Chatila", "Chazerand", 
    "Dignum", "Vayena", "Foucault", "Goffman", "Kaplan", "Haenlein", "Lee", "Brand", 
    "Manyika", "Chui", "Miremadi", "Bughin", "George", "Willmott", "Dewhurst", "McLuhan", 
    "Miller", "Erickson", "Yust", "Mitchell McCoy", "Porter", "Purdy", "Daugherty", "Romer", 
    "Stahl", "Taylor", "Vallor", "Weber", "Williamson", "Yin", "Brown", "Butzer", "Conquest", 
    "Davies", "Wheatcroft", "Garnsey", "Hassan", "Heather", "McNeill", "Mukherjee", "Nunn", 
    "Qian", "O’Grada", "Sen", "Temin", "Wickham", "Woodham-Smith"
]

# Filter: članki, ki imajo vsaj enega avtorja iz seznama referenc
# Predpostavimo, da je stolpec 'authors' v df niz z avtorji, ločenimi z vejicami
df['has_reference_author'] = df['authors'].str.lower().apply(
    lambda x: any(author.lower() in str(x) for author in reference_authors) if pd.notna(x) else False
)

# Izključimo članke, ki omenjajo zakonodajo (uporabimo obstoječi stolpec mentions_legislation)
df['reference_author_no_legislation'] = df['has_reference_author'] & ~df['mentions_legislation']

# Izpis rezultatov
print("\nČlanki z avtorji iz referenc (brez zakonodaje):", df['reference_author_no_legislation'].sum())
print(df[df['reference_author_no_legislation']][['title', 'year', 'authors']].head())

# Shranjevanje člankov v CSV
reference_author_articles = df[df['reference_author_no_legislation']][['title', 'year', 'authors', 'abstract', 'keywords']]
reference_author_articles.to_csv('reference_author_articles_no_legislation.csv', index=False)
